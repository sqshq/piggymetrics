...
function initStatisticPage() {
		...
		switch (user.checkedCurr) {
			case "rub":
				if (user.lastCurr == "rub") {
					...
					savings.freeMoney = (savings.freeMoney * user.usd).toFixed(3);
				}
				else if (user.lastCurr == "eur") {
					savings.freeMoney = (savings.freeMoney * user.eur).toFixed(3);
				}
				break;
			case "eur":
				if (user.lastCurr == "eur") {
					...
					savings.freeMoney = (savings.freeMoney * user.usd / user.eur).toFixed(3);
				}
			...
			case "usd":
				if (user.lastCurr == "usd") {
					...
					savings.freeMoney = (savings.freeMoney * user.eur / user.usd).toFixed(3);
				}
				else if (user.lastCurr == "rub") {
					savings.freeMoney = (savings.freeMoney / user.usd).toFixed(3);
				}
	...
	var initCircle = function (whichcircle, item, beforevalue) {
	...
	var initSavingsCircles = function (slidervalue, lastslidervalue, beforevalue, beforemiddlevalue, animatetime) {
		...
		if (delta >= 0) {
			if (savings.deposit) {
				if (savings.capitalization) {
					...
					monthSavings[12] = (monthSavings[0] * (1 + (savings.percent / 100)) ) + (deltaDeposit * 12) + (deltaNoDeposit * 12) + deltapercents;
				}
			}
			else {
				$('#savings-slider').css({"opacity": "0.5"}).attr('disabled', 'disabled');
	...
	switch (user.checkedCurr) {
		case "rub": $(".curr").html(" Руб ").data("curr", " р."); $(".savings-circle-currency").html(" Рублей "); $("#rubcurr").addClass("currchecked");
		break;
